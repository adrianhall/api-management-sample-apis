...
public class AzureDeveloperCliCredential : TokenCredential
{
    ...
    private static readonly Regex AzdNotFoundPattern = new("azd:(.*)not found");

    [SuppressMessage("Reliability", "CA2012:Use ValueTasks correctly", Justification = "Known and allowed usage.")]
    public override AccessToken GetToken(TokenRequestContext requestContext, CancellationToken cancellationToken = default)
        => RequestCliAccessTokenAsync(requestContext, cancellationToken).GetAwaiter().GetResult();
    ...
    public override ValueTask<AccessToken> GetTokenAsync(TokenRequestContext requestContext, CancellationToken cancellationToken = default)
        => RequestCliAccessTokenAsync(requestContext, cancellationToken);
    ...
    private async ValueTask<AccessToken> RequestCliAccessTokenAsync(TokenRequestContext context, CancellationToken cancellationToken)
    {
        try
        {
            ...
            string output = await RunProcessAsync(processStartInfo, cancellationToken);
    ...
    private static async ValueTask<string> RunProcessAsync(ProcessStartInfo processStartInfo, CancellationToken cancellationToken = default)
    {
    ...
    private static ProcessStartInfo GetAzdCliProcessStartInfo(string[] scopes)
    {
    ...
    private static AccessToken DeserializeOutput(string output)
    {
        ...
        string accessToken = root.GetProperty("token").GetString() ?? string.Empty;
